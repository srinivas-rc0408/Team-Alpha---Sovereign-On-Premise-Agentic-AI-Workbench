...
- `core/doc_diff.py` splits documents into sections on blank lines and pairs revisions by text similarity (stdlib `difflib`); its safety-critical flagging is a keyword heuristic that deliberately over-flags. It treats inspection frequencies (quarterly → annually) as thresholds just like numeric limits, and H2S controls as critical on their own. It does not flag housekeeping or procedural removals with no limit in them — the LLM safety assessment covers those. Works well on prose-style SOPs, untested on structured P&ID exports.
